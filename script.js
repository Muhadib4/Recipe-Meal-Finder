...
const categoryMeta = {
  Beef: ['🥩', 'Rich & hearty'], Chicken: ['🍗', 'Crowd favorites'], Dessert: ['🍰', 'A sweet finish'],
  Lamb: ['🍖', 'Bold & comforting'], Pasta: ['🍝', 'Always satisfying'], Seafood: ['🦐', 'Fresh from the sea'],
  Side: ['🥗', 'Perfect pairings'], Starter: ['🥣', 'Begin beautifully'], Vegan: ['🌿', 'Plant-powered'],
  Vegetarian: ['🥕', 'Full of goodness'], Breakfast: ['🍳', 'Start the day'], Goat: ['🍲', 'Deep flavors']
};
...
const categoryNames = {
  id: { Beef:'Daging sapi', Chicken:'Ayam', Dessert:'Hidangan penutup', Lamb:'Daging domba', Pasta:'Pasta', Seafood:'Hidangan laut', Side:'Pendamping', Starter:'Pembuka', Vegan:'Vegan', Vegetarian:'Vegetarian', Breakfast:'Sarapan', Goat:'Daging kambing', Rice:'Nasi', Soup:'Sup' },
  es: { Beef:'Carne', Chicken:'Pollo', Dessert:'Postre', Lamb:'Cordero', Pasta:'Pasta', Seafood:'Mariscos', Side:'Guarnición', Starter:'Entrante', Vegan:'Vegano', Vegetarian:'Vegetariano', Breakfast:'Desayuno', Goat:'Cabra', Rice:'Arroz', Soup:'Sopa' },
  fr: { Beef:'Bœuf', Chicken:'Poulet', Dessert:'Dessert', Lamb:'Agneau', Pasta:'Pâtes', Seafood:'Fruits de mer', Side:'Accompagnement', Starter:'Entrée', Vegan:'Végétalien', Vegetarian:'Végétarien', Breakfast:'Petit déjeuner', Goat:'Chèvre', Rice:'Riz', Soup:'Soupe' }
};
...
function renderMeals() {
  ...
  if (!source.length) {
    showStatus(state.view === 'favorites' ? '♡' : '⌕', state.view === 'favorites' ? t('empty') : t('noResults'), state.view === 'favorites' ? t('emptyDesc') : t('noResultsDesc'));
    return;
  }
  const visible = source.slice(0, state.shown);
  elements.recipeGrid.innerHTML = visible.map(mealCard).join('');
  elements.resultCount.textContent = t('results').replace('{shown}', visible.length).replace('{total}', source.length);
  elements.loadMore.classList.toggle('hidden', visible.length >= source.length);
}

function updateNavigation() {
  $$('.nav-link').forEach(button => button.classList.toggle('active', button.dataset.view === state.view));
...
function setHeroMeals(meals) {
  ...
  const miniCards = [['#heroMiniOne', first], ['#heroMiniTwo', second]];
  miniCards.forEach(([selector, meal]) => {
    const card = $(selector);
    $('.mini-image', card).style.backgroundImage = `url('${meal.strMealThumb}')`;
...
function updateCategorySelection() {
  $$('.category-card').forEach(button => button.classList.toggle('active', button.dataset.category === state.category));
}

function updateRegionSelection() {
  $$('.region-card').forEach(button => button.classList.toggle('active', button.dataset.region === state.region));
}
...
async function loadRegion(regionId) {
  ...
  try {
    const batches = await Promise.allSettled(region.areas.map(area => api(`filter.php?a=${encodeURIComponent(area)}`)));
    const unique = new Map();
    batches.forEach((result, index) => {
      if (result.status !== 'fulfilled') return;
      (result.value.meals || []).slice(0, 7).forEach(meal => unique.set(meal.idMeal, { ...meal, strArea: meal.strArea || region.areas[index] }));
    });
    state.meals = [...unique.values()];
    renderMeals(); scrollToRecipes();
  } catch {
...
async function getMeal(id) {
  const cached = [...state.meals, ...state.favorites].find(meal => meal.idMeal === String(id));
...
async function openRecipe(id) {
  ...
  elements.modal.showModal(); document.body.classList.add('modal-open');
  try {
    const meal = await getMeal(id);
    if (!meal) throw new Error('Recipe not found');
    const ingredients = getIngredients(meal);
    const favorite = isFavorite(meal.idMeal);
    const instructions = state.language === 'id' && meal.strInstructionsId ? meal.strInstructionsId : meal.strInstructions;
    ...
    elements.modalContent.innerHTML = '<div class="status-card"><div class="status-icon">!</div><strong>Could not load this recipe</strong><span>Close this window and try again.</span></div>';
  }
}

async function toggleFavorite(id) {
  if (isFavorite(id)) {
    state.favorites = state.favorites.filter(meal => meal.idMeal !== String(id));
  ...
  if (modalButton?.dataset.modalFavorite === String(id)) {
    ...
    modalButton.textContent = nowSaved ? `♥ ${t('saved')}` : `♡ ${t('save')}`;
  }
}
...
async function randomRecipe() {
  ...
  } catch { showToast('Could not find a recipe', '!'); }
}

async function shareRecipe(id) {
  const meal = await getMeal(id);
  const shareData = { title: `${meal.strMeal} — Savora`, text: `Try this ${meal.strMeal} recipe!`, url: location.href.split('#')[0] };
  try {
    if (navigator.share) await navigator.share(shareData);
    else { await navigator.clipboard.writeText(`${shareData.text} ${shareData.url}`); showToast('Recipe link copied', '↗'); }
  } catch (error) { if (error.name !== 'AbortError') showToast('Could not share recipe', '!'); }
}
...
elements.recipeGrid.addEventListener('click', event => {
  ...
  const card = event.target.closest('[data-id]'); if (card) openRecipe(card.dataset.id);
});
elements.recipeGrid.addEventListener('keydown', event => { if (event.key === 'Enter' && event.target.matches('[data-id]')) openRecipe(event.target.dataset.id); });
...
$('#gridView').addEventListener('click', () => { state.layout = 'grid'; localStorage.setItem('savora-layout', state.layout); $('#gridView').classList.add('active'); $('#listView').classList.remove('active'); renderMeals(); });
$('#listView').addEventListener('click', () => { state.layout = 'list'; localStorage.setItem('savora-layout', state.layout); $('#listView').classList.add('active'); $('#gridView').classList.remove('active'); renderMeals(); });
if (state.layout === 'list') { $('#listView').classList.add('active'); $('#gridView').classList.remove('active'); }

$('#randomButton').addEventListener('click', randomRecipe); $('#randomCta').addEventListener('click', randomRecipe);
...
elements.modal.addEventListener('click', event => { if (event.target === elements.modal) elements.modal.close(); });
elements.modalContent.addEventListener('click', event => { const favorite = event.target.closest('[data-modal-favorite]'); const share = event.target.closest('[data-share]'); if (favorite) toggleFavorite(favorite.dataset.modalFavorite); if (share) shareRecipe(share.dataset.share); });

$('#searchShortcut').addEventListener('click', () => elements.searchInput.focus());
document.addEventListener('keydown', event => { if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'k') { event.preventDefault(); elements.searchInput.focus(); elements.searchInput.scrollIntoView({ behavior: 'smooth', block: 'center' }); } });

elements.languageSelect.addEventListener('change', () => {
  state.language = elements.languageSelect.value;
  localStorage.setItem('savora-language', state.language);
  applyLanguage();
  showToast(state.language === 'id' ? 'Bahasa berhasil diubah' : 'Language updated', '🌐');
});
...
$('#themeToggle').addEventListener('click', () => { const dark = document.documentElement.dataset.theme === 'dark'; document.documentElement.dataset.theme = dark ? 'light' : 'dark'; localStorage.setItem('savora-theme', dark ? 'light' : 'dark'); });
window.addEventListener('scroll', () => elements.header.classList.toggle('scrolled', scrollY > 18), { passive: true });
window.addEventListener('pointermove', event => { $('#cursorGlow').style.left = `${event.clientX}px`; $('#cursorGlow').style.top = `${event.clientY}px`; }, { passive: true });

const observer = new IntersectionObserver(entries => entries.forEach(entry => { if (entry.isIntersecting) { entry.target.classList.add('visible'); observer.unobserve(entry.target); } }), { threshold: .12 });
$$('.reveal-section').forEach(section => observer.observe(section));

elements.languageSelect.value = state.language;
updateNavigation(); renderRegions(); applyLanguage(); loadFilters(); loadFeatured();
