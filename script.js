...
function renderMeals() {
  ...
  if (!source.length) {
    showStatus(state.view === 'favorites' ? '♡' : '⌕', state.view === 'favorites' ? 'Your cookbook is empty' : 'No recipes found', state.view === 'favorites' ? 'Save recipes with the heart button and they will appear here.' : 'Try a different dish, category, or cuisine.');
    return;
  }
...
async function loadFeatured({ scroll = false } = {}) {
  state.view = 'discover'; state.category = ''; state.area = ''; state.search = ''; state.shown = 8;
  elements.areaFilter.value = '';
  updateNavigation(); updateActiveFilter(); updateCategorySelection(); showSkeletons();
  elements.sectionLabel.textContent = 'Handpicked for you'; elements.sectionTitle.textContent = 'Recipes to try today';
  try {
...
async function searchMeals(term) {
  ...
  state.view = 'discover'; state.category = ''; state.area = ''; state.search = term; state.shown = 8;
  elements.areaFilter.value = '';
  updateNavigation(); updateCategorySelection(); updateActiveFilter(`Search: ${term}`); showSkeletons();
  elements.sectionLabel.textContent = 'Search results'; elements.sectionTitle.textContent = `Recipes for “${term}”`;
  try {
...
async function filterMeals(type, value) {
  state.view = 'discover'; state.shown = 8; state.search = '';
  if (type === 'c') { state.category = value; state.area = ''; elements.areaFilter.value = ''; }
  else { state.area = value; state.category = ''; }
  updateNavigation(); updateCategorySelection(); updateActiveFilter(`${type === 'c' ? 'Category' : 'Cuisine'}: ${value}`); showSkeletons();
...
async function openRecipe(id) {
  elements.modalContent.innerHTML = '<div class="status-card"><div class="status-icon">✦</div><strong>Preparing your recipe…</strong><span>Fetching ingredients and instructions.</span></div>';
...
$$('.nav-link').forEach(button => button.addEventListener('click', () => {
  ...
  if (state.view === 'favorites') { elements.sectionLabel.textContent = 'Your personal collection'; elements.sectionTitle.textContent = 'My cookbook'; renderMeals(); scrollToRecipes(); }
...
updateNavigation(); loadFilters(); loadFeatured();
